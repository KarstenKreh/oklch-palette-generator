import type { ShapeState, ShapeStyle, ShadowType, ColorMode, SeparationMode } from '@/store/shape-store';
...
export function decodeState(raw: string): Partial<ShapeState> | null {
  ...
  if (isLegacy) {
    // Prepend default style so indices align with new 21-field format
    parts.unshift('paper');
  }
