...
ssh -i "$SSH_KEY" "$SERVER" "mkdir -p $REMOTE_DIR/color-react $REMOTE_DIR/type-react $REMOTE_DIR/system-react $REMOTE_DIR/shape-react $REMOTE_DIR/symbol-react $REMOTE_DIR/space-react $REMOTE_DIR/qa-gallery $REMOTE_DIR/packages/core $REMOTE_DIR/public"
...
tar cf - --exclude='node_modules' --exclude='dist' --exclude='.git' space-react/ | ssh -i "$SSH_KEY" "$SERVER" "cd $REMOTE_DIR && rm -rf space-react && tar xf -"

# Upload qa-gallery source (hidden internal QA gallery)
echo "==> Uploading qa-gallery source..."
tar cf - --exclude='node_modules' --exclude='dist' --exclude='.git' qa-gallery/ | ssh -i "$SSH_KEY" "$SERVER" "cd $REMOTE_DIR && rm -rf qa-gallery && tar xf -"
