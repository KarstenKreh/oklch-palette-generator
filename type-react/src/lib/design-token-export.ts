 ...
 *
 * Generates JSON with typography tokens in the standardized format
 * for Figma, Style Dictionary, etc.
 */
...
export function generateDesignTokens(opts: DesignTokenOptions): string {
  const { levels, headingFont, bodyFont, monoFont, headingWeight } = opts;
